...
```php
storefrontsGetAll($types, $skip, $take, $sorting, $search, $status, $tenant_id): \Aurigma\Storefront\Model\PagedOfStorefrontDto
```
...
$status = new \Aurigma\Storefront\Model\\Aurigma\Storefront\Model\StorefrontStatus(); // \Aurigma\Storefront\Model\StorefrontStatus | Storefront status.
$tenant_id = 56; // int | Tenant identifier.

try {
    $result = $apiInstance->storefrontsGetAll($types, $skip, $take, $sorting, $search, $status, $tenant_id);
    print_r($result);
} catch (Exception $e) {
...
| **status** | [**\Aurigma\Storefront\Model\StorefrontStatus**](../Model/.md)| Storefront status. | [optional] |
| **tenant_id** | **int**| Tenant identifier. | [optional] |
